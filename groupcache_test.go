...
func TestPeers(t *testing.T) {
	...
	// redo the previous four lines of code, but with hot cache disabled to test hot cache checker func
	resetCacheSize(1 << 20)
	testGroup.checkForHotCache = HotCacheCheckerFunc(func(_ string, _ ByteView) bool { return false })
	run("base2", 200, "localHits = 49, peers = 51 49 51")
	run("base_no_hot_cache", 200, "localHits = 0, peers = 51 49 51")
	resetCacheSize(0)

	// redo again, but with hot cache always used to test hot cache checker func
	resetCacheSize(1 << 20)
	testGroup.checkForHotCache = HotCacheCheckerFunc(func(_ string, _ ByteView) bool { return true })
	run("base", 200, "localHits = 49, peers = 51 49 51")
	run("base_all_hot_cache", 200, "localHits = 0, peers = 0 0 0")
	resetCacheSize(0)
